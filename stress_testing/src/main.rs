...
fn get_current_config() -> String {
    fs::read_to_string("./stress-test-conductor.toml").expect("Unable to read file")
}
...
", pub_address, pub_address, pub_address);
    ...
    fs::write("./stress-test-conductor.toml", new_config).expect("Unable to write file");
    ...
    if persistent_directories.len() < *number_of_agents{
        for current_key in current_keys{
            ...
            if persistent_directories.contains(&current_key) == false{
                ...
                fs::create_dir(format!("{}{}", path, pub_address));
            };
        };
    };
}
...
id = \"junto-app\"
hash = \"QmZNgsWfv1LzccHMC8oSq8FrFy1gFhEYrtiYeMxDP7aq2B\"
...
[network]
type = \"n3h\"
n3h_persistence_path = \"/holochain/network_files\"

#Agent for hosting applications";

    fs::write("./stress-test-conductor.toml", general_conductor_data).expect("Unable to write file");
    ...
    for key_dir in current_keys.clone(){
        write_agent(key_dir.to_str().unwrap());
    };
